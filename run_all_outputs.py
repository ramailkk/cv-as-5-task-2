...
for ci, cpath in enumerate(CONTENT_FILES):
    ...
    if ci == 0: axes[ci, 0].set_title("Content", color="black", fontsize=10, fontweight="bold")

for si, spath in enumerate(STYLE_FILES):
    st_img = Image.open(spath).convert("RGB").resize((NST_SIZE, NST_SIZE))
    axes[0, si+1].imshow(st_img); axes[0, si+1].axis("off")
    axes[0, si+1].set_title(spath.stem.replace("style_","").capitalize(), color="#2D3748", fontsize=10, fontweight="bold")
...
fig.suptitle("NST Sanity-Check Grid  (5 content × 3 styles, β/α = 1e5)", color="black",
             fontsize=14, fontweight="bold", y=0.98)
plt.tight_layout(rect=[0, 0, 1, 0.96])
fig.savefig(OUTPUT_DIR/"grid.png", dpi=120, bbox_inches="tight", facecolor="white")
plt.close()
...
for si, spath in enumerate(STYLE_FILES):
    ...
    colors = ["black", "#2D3748", "#2F855A", "#2B6CB0", "#C53030"]
    ...
    for col, (img, title, color) in enumerate(zip(imgs, titles, colors)):
        ...
        if col == 0:
            ax.text(-15, NST_SIZE//2, spath.stem.replace("style_","").capitalize(), 
                    color="black", fontsize=14, fontweight="bold", 
                    ha="right", va="center", rotation=90)

fig.suptitle("Style Weight (β/α) Ablation Study — All Styles", color="black", fontsize=20, fontweight="bold", y=0.98)
plt.tight_layout(rect=[0, 0, 1, 0.96])
out_name = "beta_alpha_ablation.png"
fig.savefig(OUTPUT_DIR/out_name, dpi=120, bbox_inches="tight", facecolor="white")
plt.close()
...
for si, spath in enumerate(STYLE_FILES):
    ...
    panels = [
        ("Content",           Image.open(cpath).convert("RGB"),        "black"),
        ("Style",             Image.open(spath).convert("RGB"),         "#2D3748"),
        ("Shallow (Texture)", tensor_to_pil(ablation_imgs["shallow"]), "#2F855A"),
        ("Deep (Structure)",  tensor_to_pil(ablation_imgs["deep"]),    "#C53030"),
    ]
...
fig.suptitle("Style Layer Ablation Study: Shallow vs Deep", color="black", fontsize=18, fontweight="bold", y=0.98)
plt.tight_layout(rect=[0, 0, 1, 0.96])
...
col_colors = ["black", "#2F855A", "#744210"]
...
fig.text(0.5, 0.01,
         f"Matting Model (U-Net)  ·  Test-split IoU = {iou_val:.4f}  ·  Target ≥ 0.85  [ACHIEVED]",
         ha="center", color="black", fontsize=11, fontweight="bold",
         bbox=dict(boxstyle="round,pad=0.5", facecolor="#C6F6D5", edgecolor="#2F855A", lw=1.5))
fig.suptitle("Human Matting — Predicted Alpha Mattes & Cutouts", color="black",
             fontsize=14, fontweight="bold", y=0.98)
...
fig.patch.set_facecolor("white")
outer = gridspec.GridSpec(2, 2, figure=fig, wspace=0.1, hspace=0.3,
...
def plot_fm_row(gs_cell, feat: torch.Tensor, title: str, title_color: str):
    ...
    for k, idx in enumerate(idxs):
        ...
        ax.set_title(f"ch{idx}", fontsize=8, color="#4A5568")
        ax.axis("off")
    fig.text(
        gs_cell.get_position(fig).x0,
        gs_cell.get_position(fig).y1 + 0.01,
        title, color=title_color, fontsize=11, fontweight="bold",
        va="bottom", transform=fig.transFigure,
    )

plot_fm_row(outer[0,0], vf[shallow_layer], f"Video frame — {shallow_layer} (fine texture)", "#2F855A")
plot_fm_row(outer[0,1], sf[shallow_layer], f"Style image — {shallow_layer} (fine texture)", "#744210")
plot_fm_row(outer[1,0], vf[deep_layer],    f"Video frame — {deep_layer} (semantic structure)", "#2B6CB0")
plot_fm_row(outer[1,1], sf[deep_layer],    f"Style image — {deep_layer} (semantic structure)", "#C53030")

fig.suptitle(
    "VGG19 Feature Map Analysis — Shallow vs Deep Visual Representations",
    color="black", fontsize=15, fontweight="bold", y=0.95
)
...
# Use explicit fonts for premium branding
try:
...
# Title
draw2.text((40, 1024-bar_h+20), "AgriVision", fill=(255,220,80,255), font=font_title)
draw2.text((40, 1024-bar_h+75), "Neural Style Transfer  ·  Computer Vision Pipeline", fill=(240,240,240,240), font=font_main)
draw2.text((40, 1024-bar_h+110), "Human Matting  ·  VGG19 Feature Extraction  ·  Series A 2025", fill=(200,200,200,220), font=font_sub)

# Accent line
draw2.line([(30, 1024-bar_h+15), (30, 1024-15)], fill=(80,200,120,255), width=6)
